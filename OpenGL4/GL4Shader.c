...
static bool DetectUniformsAndAttributes(struct crGL4InternalShaderInfo *ShaderInfo)
	{
	GLint UniformCount, MaxUniformNameLength, UniformBlockCount;
	GLint AttributeCount, MaxAttributeNameLength;
	bool Result = false;
	...
	ShaderInfo->Attributes = calloc(AttributeCount, sizeof(struct crGL4InternalAttributeInfo));

	size_t NameLength = MAX(MaxUniformNameLength, MaxAttributeNameLength);
	char *Name = calloc(NameLength + 1, sizeof(char));

	ShaderInfo->UniformBlocks = calloc(UniformBlockCount, sizeof(struct crGL4InternalUniformInfo));
	...
	ShaderInfo->Uniforms = calloc(UniformCount, sizeof(struct crGL4InternalUniformInfo));
	...
	for (int cont = 0; cont < UniformCount; ++cont)
		...
		if (UniformSize != 1)
			...
			void *Temp = realloc(ShaderInfo->Uniforms, sizeof(struct crGL4InternalUniformInfo) * UniformCount);
			assert(Temp != NULL);
	...
	for (int cont = 0; cont < AttributeCount; ++cont)
		...
		ShaderInfo->Attributes[cont].Type = (enum crShaderAttributeType)crGL4TranslateOpenGLUniformType(GLType);
...
crShaderHandle crGL4CreateShader(struct crShaderCode NewCode)
	{
	struct crGL4InternalShaderInfo *NewShader = calloc(1, sizeof(struct crGL4InternalShaderInfo));
	assert(NewShader);
	...
	NewShader->GeneralInformation.Attributes = calloc(NewShader->AttributeCount, sizeof(struct crShaderAttributeInformation));
	...
	NewShader->GeneralInformation.Uniforms = calloc(NewShader->UniformCount, sizeof(struct crShaderUniformInformation));
	...
	NewShader->GeneralInformation.UniformBlocks = calloc(NewShader->UniformBlockCount, sizeof(struct crShaderUniformInformation));
...
bool crGL4DeleteShader(const crShaderHandle Handle)
	{
	struct crGL4InternalShaderInfo *ShaderInformation = (struct crGL4InternalShaderInfo *)pointer_list_get_node_data(Handle);
