...
static struct crRenderState CurrentState;
...
void crGL4ConfigureCulling(const struct crCullingSettings *NewSettings)
	{
...
void crGL4ConfigureFramebuffer(const crFramebufferHandle NewFramebuffer)
	...
	if (!NewFramebuffer)
		...
		{
		struct crGL4InternalFramebufferInfo *FramebufferInformation = (struct crGL4InternalFramebufferInfo *)pointer_list_get_node_data(NewFramebuffer);
