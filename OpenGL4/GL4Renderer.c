...
static bool DetectOpenGLInformation(void)
	...
	if ((glGetError() == GL_NO_ERROR) && (TempMajor != 0))
		...
		if (version_parse_string(&crGL4Information.OpenGLVersion, (const char *)GLVersionString) == false)
			return false;
		}
	...
	*/
	...
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 2, 0)) version_set(&crGL4Information.GLSLVersion, 1, 0);
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 2, 1)) version_set(&crGL4Information.GLSLVersion, 1, 2);
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 3, 0)) version_set(&crGL4Information.GLSLVersion, 1, 3);
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 3, 1)) version_set(&crGL4Information.GLSLVersion, 1, 4);
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 3, 2)) version_set(&crGL4Information.GLSLVersion, 1, 5);
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 3, 3)) crGL4Information.GLSLVersion = crGL4Information.OpenGLVersion;
	...
	if (GLSLVersionString != NULL)
		...
		if (version_parse_string(&crGL4Information.GLSLVersion, (const char *)GLSLVersionString) == false)
	...
	if (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 3, 0))
		{
	...
	crGL4Information.DirectStateAccessAvailable = (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 4, 5) || (crGL4IsExtensionAvailable("GL_ARB_direct_state_access")));
	crGL4Information.DirectStateAccessEnabled = false;

	crGL4Information.SeamlessCubeMapAvailable = (version_greater_or_equal_than(crGL4Information.OpenGLVersion, 3, 2) || (crGL4IsExtensionAvailable("ARB_seamless_cube_map")));
...
bool crGL4InitializeRenderer(const struct crRendererConfiguration NewConfiguration)
	...
	if (version_lesser_than(crGL4Information.OpenGLVersion, 4, 0))
		{
...
bool crGL4StartRenderToWindow(const crWindowHandle WindowHandle)
	...
	static struct crScissorSettings DefaultScissorSettings = { 0 };
	static struct crViewportSettings DefaultViewportSettings = { 0 };
	static struct crStencilBufferSettings DefaultStencilSettings = { 0 };
...
bool crGL4DisplayFramebuffer(const crFramebufferHandle FramebufferHandle, const crWindowHandle WindowHandle)
	...
	struct crGL4InternalFramebufferInfo *FramebufferInformation = (struct crGL4InternalFramebufferInfo *)pointer_list_get_node_data(FramebufferHandle);
...
static bool ShaderUniformValueIsEqual(const enum crShaderUniformType Type, const struct crShaderUniformValue *Value1, const struct crShaderUniformValue *Value2)
	{
	switch (Type)
		{
		case crShaderUniformType_Bool: return Value1->BoolValue == Value2->BoolValue;
		case crShaderUniformType_Bool2: return math_ivec2_equals(Value1->Bool2Value, Value2->Bool2Value);
		case crShaderUniformType_Bool3: return math_ivec3_equals(Value1->Bool3Value, Value2->Bool3Value);
		case crShaderUniformType_Bool4: return math_ivec4_equals(Value1->Bool4Value, Value2->Bool4Value);
		case crShaderUniformType_Float: return Value1->FloatValue == Value2->FloatValue;
		case crShaderUniformType_Float2: return math_vec2_equals(Value1->Float2Value, Value2->Float2Value);
		case crShaderUniformType_Float3: return math_vec3_equals(Value1->Float3Value, Value2->Float3Value);
		case crShaderUniformType_Float4: return math_vec4_equals(Value1->Float4Value, Value2->Float4Value);
		case crShaderUniformType_UnsignedInteger: return Value1->UnsignedIntegerValue == Value2->UnsignedIntegerValue;
		case crShaderUniformType_UnsignedInteger2: return math_uvec2_equals(Value1->UnsignedInteger2Value, Value2->UnsignedInteger2Value);
		case crShaderUniformType_UnsignedInteger3: return math_uvec3_equals(Value1->UnsignedInteger3Value, Value2->UnsignedInteger3Value);
		case crShaderUniformType_UnsignedInteger4: return math_uvec4_equals(Value1->UnsignedInteger4Value, Value2->UnsignedInteger4Value);
		case crShaderUniformType_Integer: return Value1->IntegerValue == Value2->IntegerValue;
		case crShaderUniformType_Integer2: return math_ivec2_equals(Value1->Integer2Value, Value2->Integer2Value);
		case crShaderUniformType_Integer3: return math_ivec3_equals(Value1->Integer3Value, Value2->Integer3Value);
		case crShaderUniformType_Integer4: return math_ivec4_equals(Value1->Integer4Value, Value2->Integer4Value);
		case crShaderUniformType_Matrix2: return math_mat2_equals(Value1->Matrix2Value, Value2->Matrix2Value);
		case crShaderUniformType_Matrix3: return math_mat3_equals(Value1->Matrix3Value, Value2->Matrix3Value);
		case crShaderUniformType_Matrix4: return math_mat4_equals(Value1->Matrix4Value, Value2->Matrix4Value);
		case crShaderUniformType_Block: return Value1->BlockValue == Value2->BlockValue;
		}

	return false;
	}

static bool CopyShaderUniformValue(const enum crShaderUniformType Type, const struct crShaderUniformValue *Input, struct crShaderUniformValue *Output)
	{
	switch (Type)
		{
		case crShaderUniformType_Bool: Output->BoolValue = Input->BoolValue; break;
		case crShaderUniformType_Bool2: math_ivec2_copy(&Output->Bool2Value, Input->Bool2Value); break;
		case crShaderUniformType_Bool3: math_ivec3_copy(&Output->Bool3Value, Input->Bool3Value); break;
		case crShaderUniformType_Bool4: math_ivec4_copy(&Output->Bool4Value, Input->Bool4Value); break;
		case crShaderUniformType_Float: Output->FloatValue = Input->FloatValue; break;
		case crShaderUniformType_Float2: math_vec2_copy(&Output->Float2Value, Input->Float2Value); break;
		case crShaderUniformType_Float3: math_vec3_copy(&Output->Float3Value, Input->Float3Value); break;
		case crShaderUniformType_Float4: math_vec4_copy(&Output->Float4Value, Input->Float4Value); break;
		case crShaderUniformType_UnsignedInteger: Output->UnsignedIntegerValue = Input->UnsignedIntegerValue; break;
		case crShaderUniformType_UnsignedInteger2: math_uvec2_copy(&Output->UnsignedInteger2Value, Input->UnsignedInteger2Value); break;
		case crShaderUniformType_UnsignedInteger3: math_uvec3_copy(&Output->UnsignedInteger3Value, Input->UnsignedInteger3Value); break;
		case crShaderUniformType_UnsignedInteger4: math_uvec4_copy(&Output->UnsignedInteger4Value, Input->UnsignedInteger4Value); break;
		case crShaderUniformType_Integer: Output->IntegerValue = Input->IntegerValue; break;
		case crShaderUniformType_Integer2: math_ivec2_copy(&Output->Integer2Value, Input->Integer2Value); break;
		case crShaderUniformType_Integer3: math_ivec3_copy(&Output->Integer3Value, Input->Integer3Value); break;
		case crShaderUniformType_Integer4: math_ivec4_copy(&Output->Integer4Value, Input->Integer4Value); break;
		case crShaderUniformType_Matrix2: math_mat2_copy(&Output->Matrix2Value, Input->Matrix2Value); break;
		case crShaderUniformType_Matrix3: math_mat3_copy(&Output->Matrix3Value, Input->Matrix3Value); break;
		case crShaderUniformType_Matrix4: math_mat4_copy(&Output->Matrix4Value, Input->Matrix4Value); break;
		case crShaderUniformType_Block: Output->BlockValue = Input->BlockValue; break;
		default:
			return false;
		}
...
static bool SetOpenGLShaderUniformValue(const struct crGL4InternalShaderInfo *ShaderInformation, const crShaderUniformHandle UniformHandle, struct crShaderUniformValue *Value, unsigned *UniformBlockBindingIndex)
	{
...
#undef CASE_TYPE_VEC
		...
		case crShaderUniformType_Block:
			{
			struct crGL4InternalShaderBufferInfo *ShaderBufferInformation = (struct crGL4InternalShaderBufferInfo *)pointer_list_get_node_data(Value->BlockValue);
...
bool crGL4RunCommand(const struct crRenderCommand Command)
	...
	if ((ActiveShader) && (ActiveShader != Command.Shader))
		{
		struct crGL4InternalShaderInfo *OldShaderInformation = (struct crGL4InternalShaderInfo *)pointer_list_get_node_data(ActiveShader);
	...
	// Activate new shader
	struct crGL4InternalShaderInfo *ShaderInformation = (struct crGL4InternalShaderInfo *)pointer_list_get_node_data(Command.Shader);
	...
	for (unsigned BindingIndex = 0; BindingIndex < Command.ShaderBufferBindingCount; ++BindingIndex)
		{
		struct crGL4InternalShaderBufferInfo *ShaderBufferInformation = (struct crGL4InternalShaderBufferInfo *)pointer_list_get_node_data(Command.ShaderBufferBindings[BindingIndex].DataStream.BufferHandle);
		struct crGL4InternalAttributeInfo *AttributeInformation = &ShaderInformation->Attributes[Command.ShaderBufferBindings[BindingIndex].AttributeHandle]; //&ShaderInformation->Attributes[BindingIndex];
	...
	for (unsigned BindingIndex = 0; BindingIndex < Command.ShaderTextureBindingCount; ++BindingIndex)
		{
		struct crGL4InternalUniformInfo *UniformInformation = &ShaderInformation->Uniforms[Command.ShaderTextureBindings[BindingIndex].UniformHandle];
		struct crGL4InternalTextureInfo *TextureInformation = (struct crGL4InternalTextureInfo *)pointer_list_get_node_data(Command.ShaderTextureBindings[BindingIndex].BindSettings.Handle);
	...
	if (Command.IndexBufferStream.BufferHandle)
		...
		struct crGL4InternalShaderBufferInfo *ShaderBufferInformation = (struct crGL4InternalShaderBufferInfo *)pointer_list_get_node_data(Command.IndexBufferStream.BufferHandle);
