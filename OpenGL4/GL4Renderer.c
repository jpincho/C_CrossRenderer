...
bool crGL4DisplayFramebuffer(const crFramebufferHandle FramebufferHandle, const crWindowHandle WindowHandle)
    ...
    crGL4InternalFramebufferInfo *FramebufferInformation = (crGL4InternalFramebufferInfo *)pointer_list_get_node_data(FramebufferHandle);
...
#undef CASE_TYPE_VEC
        ...
        case crShaderUniformType_Block:
            {
            crGL4InternalShaderBufferInfo *ShaderBufferInformation = (crGL4InternalShaderBufferInfo *)pointer_list_get_node_data(Value->BlockValue);
...
bool crGL4RunCommand(const crRenderCommand Command)
    ...
    if ((ActiveShader) && (ActiveShader != Command.Shader))
        {
        crGL4InternalShaderInfo *OldShaderInformation = (crGL4InternalShaderInfo *)pointer_list_get_node_data(ActiveShader);
    ...
    // Activate new shader
    crGL4InternalShaderInfo *ShaderInformation = (crGL4InternalShaderInfo *)pointer_list_get_node_data(Command.Shader);
    ...
    for (unsigned BindingIndex = 0; BindingIndex < Command.ShaderBufferBindingCount; ++BindingIndex)
        {
        crGL4InternalShaderBufferInfo *ShaderBufferInformation = (crGL4InternalShaderBufferInfo *)pointer_list_get_node_data(Command.ShaderBufferBindings[BindingIndex].DataStream.BufferHandle);
    ...
    for (unsigned BindingIndex = 0; BindingIndex < Command.ShaderTextureBindingCount; ++BindingIndex)
        ...
        crGL4InternalTextureInfo *TextureInformation = (crGL4InternalTextureInfo *)pointer_list_get_node_data(Command.ShaderTextureBindings[BindingIndex].BindSettings.Handle);
    ...
    if (Command.IndexBufferStream.BufferHandle)
        ...
        crGL4InternalShaderBufferInfo *ShaderBufferInformation = (crGL4InternalShaderBufferInfo *)pointer_list_get_node_data(Command.IndexBufferStream.BufferHandle);
