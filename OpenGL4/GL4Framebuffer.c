...
crFramebufferHandle crGL4CreateFramebuffer(const struct crFramebufferDescriptor Descriptor)
	{
	struct crGL4InternalFramebufferInfo *NewFramebuffer = calloc(1, sizeof(struct crGL4InternalFramebufferInfo));
	...
	if (Descriptor.DepthEnabled)
		{
		struct crTextureDescriptor NewTextureDescriptor = { 0 };
	...
	if (crGL4Information.DirectStateAccessEnabled)
		...
		if (Descriptor.DepthEnabled)
			{
			struct crGL4InternalTextureInfo *TextureInfo = (struct crGL4InternalTextureInfo *)pointer_list_get_node_data(NewFramebuffer->DepthTexture);
			glNamedFramebufferTexture(NewFramebuffer->OpenGLID, GL_DEPTH_ATTACHMENT, TextureInfo->OpenGLID, 0);
			}
		for (unsigned Index = 0; Index < Descriptor.ColorAttachments; ++Index)
			{
			struct crGL4InternalTextureInfo *TextureInfo = (struct crGL4InternalTextureInfo *)pointer_list_get_node_data(NewFramebuffer->ColorTextures[Index]);
	...
	NewNode = pointer_list_add_at_end(&crGL4Information.Framebuffers, NewFramebuffer);
	return NewNode;
...
OnError:
	...
	if (NewNode)
		pointer_list_destroy_node(&crGL4Information.Framebuffers, NewNode);
	return FramebufferHandle_Invalid;
	}

bool crGL4DeleteFramebuffer(const crFramebufferHandle Handle)
	{
	struct crGL4InternalFramebufferInfo *FramebufferInformation = (struct crGL4InternalFramebufferInfo *)pointer_list_get_node_data(Handle);
	...
	pointer_list_destroy_node(&crGL4Information.Framebuffers, Handle);
	return true;
	}
