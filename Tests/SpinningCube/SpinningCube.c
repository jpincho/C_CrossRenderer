...
int main(int argc, char *argv[])
	...
	mat4 ViewMatrix;
	math_mat4_set_view_matrix(&ViewMatrix, (vec3) { 0.0f, 0.0f, 2.0f }, (vec3) { 0.0f, 0.0f, 0.0f }, (vec3) { 0, 1.0f, 0 });

	struct crShaderBufferDescriptor Descriptor;
	...
	Descriptor.Capacity = sizeof(Indices);
	Descriptor.Data = &Indices;
	Descriptor.DataSize = sizeof(Indices);
	crShaderBufferHandle IndexShaderBufferHandle = crCreateShaderBuffer(Descriptor);
	...
	struct crFramebufferDescriptor FramebufferDescriptor = { 0 };
	...
	struct crShaderBufferDataStream PositionStream;
	...
	struct crShaderBufferDataStream TexCoordStream;
	...
	struct crShaderBufferDataStream IndexStream;
